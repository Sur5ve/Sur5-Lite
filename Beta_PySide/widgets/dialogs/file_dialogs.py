...
def show_save_conversation_dialog(parent: QWidget = None) -> Optional[str]:
    ...
    """
    default_dir = str(Path.home() / "Documents" / "BetaVersion" / "Conversations")
    
    filepath, _ = QFileDialog.getSaveFileName(
        parent,
        "Save Conversation",
        default_dir,
        "Beta Chat Files (*.betachat);;All Files (*.*)",
        "Beta Chat Files (*.betachat)"
    )
    
    if filepath:
        # Ensure .betachat extension
        if not filepath.endswith('.betachat'):
            filepath += '.betachat'
...
def show_export_text_dialog(parent: QWidget = None) -> Optional[str]:
    ...
    Returns:
        Selected filepath or None if cancelled
    """
    default_dir = str(Path.home() / "Documents" / "BetaVersion" / "Exports")
